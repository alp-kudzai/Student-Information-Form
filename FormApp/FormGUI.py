...
def main():
    ...
    def buildForm(values, studentForm, full_name, Storage):
        ...
        form_field = ['Birthday', 'Parent1', 'Parent1 Numbers','Parent2', 
    ...
    def updatebuild(values, studentForm, full_name, Storage):
        keys = ['BRTH', 'P1FN', 'P1CN', 'P2FN', 'P2CN', 'SK', 'FAM', 'HL', 'FF',
                'FC', 'PA']
        dict_formField = {'BRTH':'Birthday',
                          'P1FN':'Parent1',
                          'P1CN':'Parent1 Numbers',
                          'P2FN':'Parent2',
                          'P2CN':'Parent2 Numbers',
                          'SK':'Skills',
                          'FAM':'Ambitions',
                          'HL':'Hobbies',
                          'FF':'Fav Food',
        ...
        for k in keys:
            if len(values[k]) > 0:
                ...
                keys_filled.append(k)
        # print(listOfilled)
        # print(keys_filled)
        for i in range(len(listOfilled)):
            Storage.update_STUDinfo(full_name,dict_formField[keys_filled[i]], listOfilled[i])
        return Storage
